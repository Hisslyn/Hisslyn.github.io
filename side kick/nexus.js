...
word.split('').forEach((ch, i) => {
  ...
  s.style.animationDelay = (0.35 + i * 0.07) + 's'; title.appendChild(s);
});
...
// distant stars
const starGeo = new THREE.BufferGeometry(); const SC = 1600; const sp = new Float32Array(SC * 3);
for (let i = 0; i < SC; i++) { sp[i * 3] = (Math.random() - 0.5) * 140; sp[i * 3 + 1] = (Math.random() - 0.5) * 140; sp[i * 3 + 2] = (Math.random() - 0.5) * 140; }
...
for (let i = 0; i < ASTN; i++) {
  ...
  for (let k = 0; k < pos.count; k++) { const f = 0.85 + Math.random() * 0.3; pos.setXYZ(k, pos.getX(k) * f, pos.getY(k) * f, pos.getZ(k) * f); }
  g.computeVertexNormals();
  const m = new THREE.Mesh(g, astMat); m.position.copy(spawnPos()); scene.add(m);
  ...
  scene.add(trail);
  asteroids.push({ m, trail, tlen: 3 + Math.random() * 6, speed: 0.7 + Math.random() * 0.8,
...
(function buildMilkyWay() {
  const W = 2048, H = 1024; const cv = document.createElement('canvas'); cv.width = W; cv.height = H;
  ...
  for (let i = 0; i < 9000; i++) {
    ...
    const w = Math.random(); x.fillStyle = w < 0.7 ? '#eaf0ff' : (w < 0.9 ? '#acc4ff' : '#c9b8ff');
  ...
  window.__milkyway = sky; scene.add(sky);
})();
...
sunCorona.forEach(([s, c, o]) => {
  ...
  sp.scale.set(s, s, 1); sp.userData.base = s; sp.userData.o = o; sp.userData.corona = true; sunGrp.add(sp);
});
const sunLight = new THREE.PointLight(0xffd9a0, 3.6, 340); sunGrp.add(sunLight);

// ---- sun flares ----
const flareStarTex = (function () {
  const cv = document.createElement('canvas'); cv.width = cv.height = 256; const x = cv.getContext('2d');
  x.translate(128, 128);
  const rays = [[0, 124], [Math.PI / 2, 124], [Math.PI, 124], [Math.PI * 1.5, 124],
    [Math.PI / 4, 66], [Math.PI * 3 / 4, 66], [Math.PI * 5 / 4, 66], [Math.PI * 7 / 4, 66]];
  rays.forEach(([ang, len]) => {
    x.save(); x.rotate(ang);
...
flareStar.scale.set(30, 30, 1); sunGrp.add(flareStar);
...
function planetTexture(base, accent, bands) {
  ...
  for (let i = 0; i < 2400; i++) {
    ...
    x.fillStyle = Math.random() > 0.5 ? accent : '#000'; x.beginPath(); x.arc(Math.random() * 256, Math.random() * 256, r, 0, 7); x.fill();
  }
  if (bands) {
    for (let y = 0; y < 256; y += 4) {
      x.globalAlpha = 0.05 + Math.random() * 0.08;
      x.fillStyle = Math.random() > 0.5 ? accent : base; x.fillRect(0, y + Math.sin(y * 0.3) * 2, 256, 3);
    }
  }
...
// ---- planets ----
const planetDefs = [
  { name: 'Work',    sub: 'Things I have built',    href: 'page1.html', dist: 26, incl: 0.18,  node: 0.4, phase: 0.6, speed: 0.04,  r: 2.4, base: '#16486e', accent: '#5cc8ff', spin: 0.22, ring: false, tilt: 0.4 },
  { name: 'About',   sub: 'Who is behind this',     href: 'page2.html', dist: 44, incl: 0.45,  node: 2.1, phase: 2.4, speed: 0.026, r: 3.8, base: '#8a5a1e', accent: '#ffce5e', spin: 0.12, ring: true,  tilt: 0.6 },
  { name: 'Lab',     sub: 'Works in progress',      href: 'page3.html', dist: 64, incl: -0.3,  node: 4.0, phase: 4.1, speed: 0.016, r: 1.5, base: '#0e5a55', accent: '#3df0d0', spin: 0.34, ring: false, tilt: 0.2 },
  { name: 'Contact', sub: 'Start a conversation',   href: 'page4.html', dist: 14, incl: 0.6,   node: 5.2, phase: 1.3, speed: 0.055, r: 2.0, base: '#3a3470', accent: '#9d8cff', spin: 0.18, ring: false, tilt: 0.9 },
];
...
planetDefs.forEach((d, i) => {
  ...
  const seg = 128, pts = [];
  for (let s = 0; s <= seg; s++) { const a = s / seg * Math.PI * 2; pts.push(new THREE.Vector3(Math.cos(a) * d.dist, 0, Math.sin(a) * d.dist)); }
  ...
  const gScale = d.r * 4.2; glow.scale.set(gScale, gScale, 1);
  glow.userData.base = gScale;
  grp.add(glow);

  if (d.ring) {
    const rg = new THREE.Mesh(new THREE.RingGeometry(d.r * 1.5, d.r * 2.4, 64),
      new THREE.MeshBasicMaterial({ color: d.accent, transparent: true, opacity: 0.22, side: THREE.DoubleSide }));
    rg.rotation.x = Math.PI / 2.2; grp.add(rg);
  }
...
function animate() {
  ...
  if (!dragging && !warp) {
    theta += velT; phi += velP;
    phi = Math.max(PHI_MIN, Math.min(PHI_MAX, phi)); velT *= 0.94; velP *= 0.94; theta += 0.0006 * SYS;
  }
  if (warp) {
