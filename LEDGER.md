...
| docs-stale | build | BUILT | — | `tools/docs-stale.py` exists and `npm run docs:stale` is wired in `package.json` (script line 10). Reconciled 2026-07-26 against disk |
...
| verify-later | process | BUILT | — | `VERIFY-LATER.md` exists on disk, populated with an Entries section (one live entry keyed to `d92605a`), Rules, and a Burn-down procedure. Reconciled 2026-07-26 against disk |
| loop-prompts | process | BUILT | — | `loop-prompts/` holds `loopA.md`, `loopB.md`, `loopD.md`, `loopE.md`, `_closing-block.txt` — matches "Loop C deliberately excluded" exactly. Reconciled 2026-07-26 against disk |
| invariant-hook | process | BUILT | — | `.claude/hooks/invariant-guard.py` + `.claude/hooks/invariant-guard.config.json` both present on disk. Reconciled 2026-07-26 against disk |
| session-start-hook | process | BUILT | — | `.claude/hooks/session-start.py` present on disk (executable). Reconciled 2026-07-26 against disk. `verify_later_summary()` **diverges from the kit copy** and must not be reverted to it: the kit parser skipped every line not starting with `-`, so it matched none of this project's documented `` `<commit>` — <action> — <expected> `` entries and reported 0 pending against a growing backlog — the precise failure the counter exists to prevent. Now scoped to the `## Entries` section, matches backticked-SHA and `PENDING` entries, still accepts the kit's `-` list form, and classifies by explicit `SUPERSEDED`/`CONFIRMED` marker before strike-through (burn-down strikes confirmed entries through too). Kit bug — worth fixing upstream in `claude-project-kit` |
| turn-end-hook | process | BUILT | — | `.claude/hooks/turn-end.py`, wired as a `Stop` hook. Prints commit SHA, working-tree file count, LEDGER B/P/M, verify-later pending, and build staleness (any build source newer than its `.min` twin) after every turn, plus a `changed:` delta line. Harness-generated, so it cannot be narrated into existence — it is the check on the agent's own claims, and its absence for a turn means the canary is dead rather than quiet. Reuses `session-start.py`'s parsers via importlib (with `sys.dont_write_bytecode`, or the `__pycache__` it writes dirties the tree it measures) and derives build pairs from `package.json`, so no path or count is hardcoded. Appends to `~/.claude/turn-state/<hash>.log` — outside the repo, since a state file inside it would be publicly served *and* would dirty the working tree it reports on. **Session identity (2026-07-27):** every session gets an ordinal `S-NN`, assigned on its first turn and held in `<hash>.sessions.json` as `{"next", "ids"}` — `next` is stored rather than derived so trimming the id map cannot recycle a number some older log line already answers to. The ordinal exists because Claude Code writes an `ai-title` record **once**, from the opening prompt, and never reconsiders: measured across this project's transcripts every session carries at most one distinct `ai-title` value, re-persisted per turn, and a one-word opener (`test`, `hi`) is declined outright and never gets the record at all. That is narrower than "the session has no name" — the VS Code session list still labels an untitled session, apparently deriving one at display time from its first substantial prompt, but nothing stores that label, so the canary and the session list can disagree. `4d0afed` overstated this as "nameless for life"; corrected in the commit that follows it. A `--name` override that let a session be titled by hand was added and then **removed the same day**: a hand-set string renders identically to a real title, so the canary line asserted a name that existed nowhere but the hook's own state file — precisely the class of unverifiable claim this hook exists to catch. There is deliberately no way to name a session by hand; an untitled one shows its ordinal and id, both of which are true. Log entries are blank-line separated, because a turn that moved something is two lines and the separator is what makes one turn visible |
| slash-commands | process | BUILT | — | `.claude/commands/` held 8 files as of the 2026-07-26 reconcile pass; `loop-c.md` was then deleted (dead command, invoked a nonexistent `loop-prompts/loopC*`) — 7 remain. Reconciled against disk |
| docs-index | process | BUILT | docs-stale | First documentarian pass complete: `docs/_manifest.json` now holds real entries (read the file for the current count) covering the JS behaviour layer (`src/js/audio.js`, `timer.js`, `lang.js`, `drawer.js`, `secret.js`, `nexus.js`) plus `universe/nexus.js`, `tools/check-site.py`, and `tools/docs-stale.py`, each with a per-file doc under `docs/` and a real sha256 hash verified against `npm run docs:stale` (clean run plus a corrupt-and-restore proof, both passed 2026-07-26). `docs/INDEX.md` lists all of them. Every other source file in the repo remains undocumented — this pass was scoped to the JS behaviour layer and tooling only, not a full sweep |
| references-bibles | process | PARTIAL | — | `references/` holds only `README.md` on disk — no bibles. PARTIAL by design per the 2026-07-26 reconcile ruling |
